...
function LatencyMonitor({ latency, localTotalMs }: { latency: LatencyStats; localTotalMs: number | null }) {
  ...
  const displayMs = backendTotal > 0 ? backendTotal * 1000 : (localTotalMs ?? null);
  const displayStr = displayMs == null ? "—" : displayMs >= 1000 ? `${(displayMs / 1000).toFixed(1)}s` : `${displayMs}ms`;
...
function DailyUsageChart({ data, dayFilter, onChangeFilter }: any) {
  ...
  const peak = data.reduce((a: any, b: any) => (a.count > b.count ? a : b), { count: 0 });
  ...
  return (
    <div className="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm h-full flex flex-col">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        ...
        <div className="bg-gray-50 rounded-2xl p-4 border border-gray-100">
          <div className="text-[10px] text-gray-400 font-bold">AVG</div>
          <div className="text-xl font-black text-gray-900">{avg.toFixed(1)}</div>
        </div>

        <div className="bg-gray-50 rounded-2xl p-4 border border-gray-100">
          <div className="text-[10px] text-gray-400 font-bold">PEAK</div>
          <div className="text-xl font-black text-gray-900">{peak.count}</div>
        </div>
      </div>
          ...
          data.map((d: any, idx: number) => {
            ...
            return (
              ...
              >
                ...
                {/* 값 */}
                <div className="text-sm font-black w-10 text-right">
                  {d.count}
                </div>
              </div>
            );
